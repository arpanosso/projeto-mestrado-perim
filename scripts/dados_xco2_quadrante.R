...
geom_sf(data = brasil, fill = "lightgray", color = "black") +
  ...
  # Usamos unlist(year) para converter a coluna-lista em um vetor simples
  # antes de comparar com 2020.
  geom_sf(data = dados_no_quadrante %>% filter(unlist(year) == 2020), aes(color = xco2), size = 0.5) +
  ...
  # adiciona títulos
  labs(
    title = "Valores de Concentração de xco₂ em 2020",
    subtitle = "",
    x = "Longitude",
    y = "Latitude",
    color = "xco2 (ppm)"
