...
get_geobr_state_muni <- function(x1,y1,df){
  df_aux <- df |> 
    dplyr::mutate(distancia = sqrt((x1-df$lon_centroide)^2+(y1-df$lat_centroide)^2)) |> 
    dplyr::arrange(distancia) |> 
    dplyr::slice(1:10)
  list_pol <- purrr::map(1:10, ~{
...
};get_geobr_state_muni(-45.92468,-16.158712,municipality_df)
```
